...
// ─── Session form (shared between add + edit modal) ───────────────────────────
...
function SessionForm({
  ...
  // Auto-select first task when contract changes
  useEffect(() => {
    ...
    <form onSubmit={(e) => void handleSubmit(e)} className="grid gap-4">
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        ...
        <Field label="Contract">
          <select value={contractId} onChange={(e) => setContractId(e.target.value)} className={inputClass} required>
            {contracts.map((c) => (
        ...
        <Field label="Taak">
          <select value={taskId} onChange={(e) => setTaskId(e.target.value)} className={inputClass} required>
        ...
        <Field label="Inkloktijd">
          <input type="time" value={clockIn} onChange={(e) => setClockIn(e.target.value)} className={inputClass} required />
        </Field>
        <Field label="Uitkloktijd">
          <input type="time" value={clockOut} onChange={(e) => setClockOut(e.target.value)} className={inputClass} required />
        </Field>
        <Field label="Pauze (minuten)">
          <input
            type="number"
            min={0}
            value={pauseMinutes}
        ...
        <Field label="Berekende uren">
          <div className={cn(inputClass, "flex items-center font-semibold", computedHours ? "text-[var(--primary)]" : "text-[var(--muted)]")}>
            {computedHours != null ? formatHours(computedHours) : "—"}
          </div>
        </Field>
      </div>
      <Field label="Notities / omschrijving" className="col-span-2">
        <input
          type="text"
          value={notes}
...
function ClockOutModal({
  ...
  const contract = contracts.find((c) => c.id === session.contractId);
  ...
  async function handleConfirm(e: React.FormEvent) {
    ...
    const fd = new FormData();
    fd.append("employeeId", employeeId);
    fd.append("contractId", session.contractId);
    fd.append("taskId", session.taskId);
    fd.append("date", date);
    fd.append("clockIn", clockIn);
    fd.append("clockOut", clockOut);
  ...
  return (
    <Modal open={open} onClose={onClose} title="Sessie afsluiten">
      <form onSubmit={(e) => void handleConfirm(e)} className="grid gap-4">
        ...
        <div className="rounded border border-[var(--border)] bg-slate-50 p-3 text-sm grid gap-1">
          ...
          {contract && (
            <div className="flex gap-4">
              ...
              <span className="font-medium">{contract.code} — {contract.name}</span>
            </div>
          )}
          {task && (
            <div className="flex gap-4">
              <span className="text-[var(--muted)]">Taak</span>
        ...
        <div className="grid gap-3 sm:grid-cols-3">
          ...
          <Field label="Pauze (min)">
            <input type="number" min={0} value={pauseMinutes} onChange={(e) => setPauseMinutes(e.target.value)} className={inputClass} required />
          </Field>
        </div>
        <Field label="Berekende uren">
          <div className={cn(inputClass, "flex items-center font-semibold", computedHours ? "text-[var(--primary)]" : "text-red-600")}>
            {computedHours != null ? formatHours(computedHours) : "Ongeldige tijden"}
          </div>
        </Field>
        <Field label="Werk / omschrijving">
          <input type="text" value={notes} onChange={(e) => setNotes(e.target.value)} className={inputClass} placeholder="Wat heb je gedaan?" />
        </Field>
        <div className="flex justify-end gap-2 pt-1">
          <Button type="button" variant="secondary" onClick={onClose}>Annuleren</Button>
...
export function HoursTracker({ employees, contracts, tasks, entries }: Props) {
  ...
  useEffect(() => {
    ...
    const storedCon = localStorage.getItem("tracker_contract") ?? "";
    const storedTask = localStorage.getItem("tracker_task") ?? "";

    const emp = employees.find((e) => e.id === storedEmp) ? storedEmp : (employees[0]?.id ?? "");
    const con = contracts.find((c) => c.id === storedCon) ? storedCon : (contracts[0]?.id ?? "");
    const firstTask = tasks.filter((t) => t.contractId === con)[0]?.id ?? "";
    const task = tasks.find((t) => t.id === storedTask) ? storedTask : firstTask;
    ...
    if (emp) {
      const raw = localStorage.getItem(`tracker_active_${emp}`);
      if (raw) {
        try {
  ...
  const handleEmployeeChange = useCallback((id: string) => {
    setEmployeeId(id);
    if (typeof window !== "undefined") localStorage.setItem("tracker_employee", id);
    // load session + goal for new employee
    if (typeof window !== "undefined") {
      const raw = localStorage.getItem(`tracker_active_${id}`);
      if (raw) {
        try { setSession(JSON.parse(raw) as ActiveSession); } catch { setSession(null); }
      } else {
        setSession(null);
      }
  ...
  const handleContractChange = useCallback((id: string) => {
    setContractId(id);
    if (typeof window !== "undefined") localStorage.setItem("tracker_contract", id);
    const firstTask = tasks.filter((t) => t.contractId === id)[0]?.id ?? "";
    setTaskId(firstTask);
    if (typeof window !== "undefined") localStorage.setItem("tracker_task", firstTask);
  }, [tasks]);

  const handleTaskChange = useCallback((id: string) => {
    setTaskId(id);
    if (typeof window !== "undefined") localStorage.setItem("tracker_task", id);
  }, []);

  const filteredTasks = tasks.filter((t) => t.contractId === contractId);

  // ── Session net elapsed (ms) ────────────────────────────────────────────────
  const netElapsedMs = (() => {
    if (!session) return 0;
    const currentPause = session.isPaused && session.pauseStartedTs
      ? now - session.pauseStartedTs
      : 0;
  ...
  function handleClockIn() {
    if (!employeeId || !contractId || !taskId) return;
    ...
    if (typeof window !== "undefined") {
      localStorage.setItem(`tracker_active_${employeeId}`, JSON.stringify(newSession));
    }
  }
  ...
  function handleTogglePause() {
    ...
    if (session.isPaused) {
      ...
      updated = {
        ...
        isPaused: false,
        pauseStartedTs: null,
        pausedAccumMs: session.pausedAccumMs + elapsed,
      };
    } else {
      // Pause
    ...
    setSession(updated);
    if (typeof window !== "undefined") {
      localStorage.setItem(`tracker_active_${employeeId}`, JSON.stringify(updated));
    }
  }

  // ── Clock out → open confirm modal ──────────────────────────────────────────
  function handleClockOut() {
    setShowClockOutModal(true);
  }

  function handleClockOutConfirmed() {
    if (typeof window !== "undefined") {
      localStorage.removeItem(`tracker_active_${employeeId}`);
    }
    setSession(null);
  ...
  // ── Werklog entries for selected month ─────────────────────────────────────
  ...
  return (
    <div className="grid gap-5">
      {/* ── Identity selector ─────────────────────────────────────────────── */}
      <Card>
        <CardHeader title="Urentracker" description="Kies je medewerker, contract en taak om in te klokken." />
        <div className="grid gap-3 sm:grid-cols-3">
          <Field label="Medewerker">
          ...
          <Field label="Contract">
            <select value={contractId} onChange={(e) => handleContractChange(e.target.value)} className={inputClass} disabled={noEmployee}>
              {contracts.map((c) => (
                <option key={c.id} value={c.id}>{c.code} — {c.name}</option>
              ))}
            </select>
          ...
          <Field label="Taak">
            <select value={taskId} onChange={(e) => handleTaskChange(e.target.value)} className={inputClass} disabled={noEmployee || filteredTasks.length === 0}>
              {filteredTasks.length === 0
      ...
      {noEmployee ? (
        <Card>
          <div className="flex flex-col items-center gap-2 py-6 text-center text-[var(--muted)]">
            ...
            <p className="text-sm">Kies een medewerker om de tracker te gebruiken.</p>
          </div>
        </Card>
      ) : (
        <>
              ...
              <div className="flex items-center gap-2">
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (typeof window !== "undefined") {
                      localStorage.setItem(`tracker_goal_${employeeId}`, e.target.value);
                    }
                  }}
