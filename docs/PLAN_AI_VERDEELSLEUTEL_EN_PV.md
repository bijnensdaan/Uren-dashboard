...
- **Document-export (.docx + .xlsx) geïmplementeerd.** Naast de printbare HTML-PV genereert het dashboard nu echte downloadbare bestanden in de structuur van de docs/-referenties:
  - `lib/domain/pv-docx.ts` (library `docx`): bouwt het proces-verbaal als Word-bestand — logo (`public/pv-logo.png`, overgenomen uit het referentiebestand), kopblok, "Inzet van"/"Ter realisatie van", de twee vaste alinea's, de Facturatie-tabel met exact dezelfde 7 kolommen en kolombreedtes (twips uit het origineel), de drie financiële regels, datum en het twee-koloms handtekeningblok. A4, Calibri 11, marges overgenomen uit het origineel.
  - `lib/domain/pv-xlsx.ts` (library `xlsx`/SheetJS): genereert het uren-Excel — tabblad "Gepresteerde uren" (medewerker × datum-matrix met eindtotalen, uit de time entries) en tabblad "Overzicht bedragen" (de facturatietabel + reeds gefactureerd / nu te factureren / nog te factureren). Geverifieerd tegen het AVSA24-bestand: bedragen identiek.
  - Download-routes `app/api/reports/[id]/pv-docx` en `.../uren-xlsx` (Node-runtime) + exportknoppen "PV (Word)" en "Uren (Excel)" op de rapportpagina. Gedeelde dataverzameling in `lib/pv-export-server.ts` (zelfde auto-fill/berekeningslogica als de pagina). **Gemini levert alleen de tekst** (deliverables + vaste alinea's); de bestanden zelf en alle cijfers zijn deterministisch.
  - Beperking: docx-js/SheetJS reproduceren de **structuur, tekst, tabel en bedragen** getrouw, maar niet elk pixel-/stijldetail (lettertype-finesse, celranden/kleuren in Excel) van het originele overheidssjabloon.
- **Nog niet gedaan in Stap 2**: het automatische aanmaken van de `Invoice` is een aparte knopactie (geen automatische koppeling bij PV-goedkeuring), en de tarieven/btw/namen staan nu zowel op contractniveau (`ProfileRate`/`Contract`) als overschrijfbaar per PV (`pvDataJson`).
